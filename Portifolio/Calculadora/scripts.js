...
class Calculator {
  ...
  updateDisplay() {
    this.previousOperandTextElement.innerText = `${this.formatDisplaynumero(
      this.previousOperand
    )} ${this.operation || ""}`;
    this.currentOperandTextElement.innerText = this.formatDisplaynumero(
      this.currentOperand
    );
  }
}
...
for (const numeroButton of numeroButtons) {
  numeroButton.addEventListener("click", () => {
    calculator.appendnumero(numeroButton.innerText);
